...
void VulkanTexture::create(const VulkanDevice& vDevice, const VulkanCommandBuffer& cmd, const std::string& path)
{
...
void VulkanTexture::createWhitePixel(const VulkanDevice& vDevice, const VulkanCommandBuffer& cmd)
{
    this->mipLevels = 1;
...
void VulkanTexture::createImage(const VulkanDevice& vDevice, uint32_t width, uint32_t height, 
...
void VulkanTexture::createTextureImageView(VkDevice device, VkFormat format, uint32_t mipLevels)
{   
    imageView = vkUtils::createImageView(device, image, 
        VK_FORMAT_R8G8B8A8_SRGB, VK_IMAGE_ASPECT_COLOR_BIT, mipLevels);
}
    
void VulkanTexture::createSampler(const VulkanDevice& vDevice)
{
    ...
    void VulkanTexture::transitionImageLayout(const VulkanDevice& vDevice, 
    ...
    void VulkanTexture::copyBufferToImage(const VulkanDevice& vDevice, const VulkanCommandBuffer& cmd, VkBuffer buffer, VkImage image, uint32_t width, uint32_t height)
    {
        VkCommandBuffer commandBuffer = beginSingleTimeCommands(vDevice, cmd);
...
void VulkanTexture::generateMipmaps(const VulkanDevice& vDevice, const VulkanCommandBuffer& cmd, VkImage image,  VkFormat imageFormat,
